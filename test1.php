...
function f6(){
	tbt_level0_zoom(tbt_level0[0][1][0],"off");
}
function f7(){

	/*t_ajax_ajs("l1_home_p1");
	tbt_ajax_ajs("l1_blog_p1");
	tbt_ajax_ajs("l1_blog_p2");*/
}
function f8(){
...
function init(){
	...
	var tbt_level1_el=["l1_home_p1"];
	tbt_level1_el.push(<?php echo $init_levels->level1_init("l1_home_p1"); ?>);
...
function tbt_level0_delete(el){
	...
	del_el = new TimelineMax({onComplete:tbt_remove_element,onCompleteParams:[id_div]});
 	del_el.add(TweenLite.to("#"+id_div,.5, { autoAlpha:0,ease: Linear.easeNone}));
	del_el.add(TweenLite.to("#signal_"+id_div,.5, { autoAlpha:0,ease: Linear.easeNone}));
...
function tbt_level0_create(el){
	...
	//var tl = new TimelineMax({onComplete:tbt_signal_create,onCompleteParams:[id_div,id_color,coord_x,coord_y,id_w,id_h]});
   	var tl = new TimelineMax({onComplete:tbt_level0_create_complete,onCompleteParams:[el]});
 	tl.fromTo( "#"+id_div,2,{x:Math.ceil(coord_x_s),y:Math.ceil(coord_y_s),width:w_s+"%",height:h_s+"%"},{x:Math.ceil(coord_x),y:Math.ceil(coord_y),width:id_w+"%",height:id_h+"%",border:border_mode,ease: Expo.easeOut});
 }
	...
	// check if is a timer function 
 	if(el[10]){
		...
		aj_as = new TimelineMax();
	 	...
	 	aj_as.add(function(){ jQuery("#inner_"+id_div).append(data);});
...
var main_url_ajs = "http://192.168.0.31/dev/TBT/div-elements/ajax_json.php";
...
function tbt_signal_create(el){ 
	...
	sfcoord_x=Math.ceil((sid_x_px + sid_w_px) - signal_w)-4;
	//sfcoord_x=((id_x==0)?(id_x + sid_w_px):id_x) - signal_w;
	sfcoord_y=Math.ceil((sid_y_px + sid_h_px ) - signal_h)+1; 
 	TweenLite.set(jQuery(signal_id), { opacity:0,borderRightColor:"transparent",borderLeftColor:"transparent",borderTopColor:"transparent",borderBottomColor: id_color,position:"absolute", x:sfcoord_x+"px", y:sfcoord_y+"px"});
 	// create poligon
